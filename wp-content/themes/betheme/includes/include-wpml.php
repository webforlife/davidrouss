...
if (has_nav_menu('lang-menu')) {
	...
	echo '<div class="wpml-languages custom">';
		echo '<a class="active" href="#">'. esc_html(mfn_get_menu_name('lang-menu')) .'<i class="icon-down-open-mini" aria-hidden="true"></i></a>';
		mfn_wp_lang_menu();
	echo '</div>';
		...
		if (! $wmpl_flags || $wmpl_flags == 'dropdown-name') {
			...
			if ($active_lang) {
				...
				echo '<div class="wpml-languages '. esc_attr($lang_status) .'">';
					...
					echo '<a class="active" tabindex="0" data-title="wpml" aria-label="'. __('Language menu icon', 'betheme') .'">';
					...
					if (count($languages)) {
						echo '<ul class="wpml-lang-dropdown" aria-expanded="false">';
