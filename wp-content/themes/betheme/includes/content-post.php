	...
	function mfn_content_post( $query = false, $style = false, $attr = false ){
		...
		if ( $query->have_posts() ){
			while ( $query->have_posts() ){
				...
				$output .= '<article class="'. esc_attr($post_class) .'" style="'. esc_attr($bg_color) .'">';
					...
					if( 'masonry tiles' == $style ){
						...
						if( get_post_format() == 'video' ){
							...
							$output .=  '<i class="post-format-icon icon-play" aria-hidden="true"></i>';

						} elseif( get_post_format() == 'quote' ){

							$output .=  '<i class="post-format-icon icon-quote" aria-hidden="true"></i>';

						} elseif( get_post_format() == 'link' ){

							$output .=  '<i class="post-format-icon icon-link" aria-hidden="true"></i>';

						} elseif( get_post_format() == 'audio' ){	// for future use

							$output .=  '<i class="post-format-icon icon-music-line" aria-hidden="true"></i>';
							...
							if( $rev_slider || $lay_slider ){
								$output .=  '<i class="post-format-icon icon-code" aria-hidden="true"></i>';
							}
					...
					if( $bg_color || '#ffffff' != mfn_opts_get('background-archives-post','') ){
						$item_bg_class .= ' has-custom-bg';
					}
						...
						$output .= '<div class="post-desc">';
									...
									$output .= '<div class="post-meta clearfix">';
										...
										$output .= '<div class="author-date">';
											...
											if( isset( $list_meta['author'] ) ){
												$output .= '<span class="vcard author post-author">';
													...
													$output .= '<i class="icon-user" aria-label="'. __('author', 'betheme') .'"></i> ';
											...
											if( 'masonry tiles' == $style && comments_open() && mfn_opts_get( 'blog-comments' ) ){
												$output .= '<div class="post-links">';
													$output .= '<i class="icon-comment-empty-fa" aria-label="'. __('number of comments', 'betheme') .'"></i> <a href="'. esc_url(get_comments_link()) .'" class="post-comments">'. esc_html(get_comments_number()) .'</a>';
												$output .= '</div>';
											}
										...
										if( isset( $list_meta['categories'] ) ){
											$output .= '<div class="category">';
												$output .= '<span class="cat-btn">'. esc_html($translate['categories']) .' <i class="icon-down-dir" aria-hidden="true"></i></span>';
								...
								if( 'photo' == $style ){
									$output .= '<div class="post-footer">';
										...
										$output .= '<div class="post-links">';
											...
											$output .= '<i class="icon-doc-text" aria-hidden="true"></i> <a href="'. esc_url(get_permalink()) .'" class="post-more">'. $button_text .'</a>';
										$output .= '</div>';
							...
							$output .= '<div class="post-title">';
								...
								if( get_post_format() == 'quote' ){
									...
									$output .= '<i class="icon-link" aria-hidden="true"></i>';
									...
									$output .= '<'. mfn_allowed_title_tag($title_tag) .' class="entry-title '. esc_attr($title_class) .'" itemprop="headline"><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></'. mfn_allowed_title_tag($title_tag) .'>';
							...
							if( 'photo2' == $style ){
								if( isset( $list_meta['author'] ) || isset( $list_meta['date'] ) ){
									$output .= '<div class="post-footer">';
										...
										if( isset( $list_meta['author'] ) ){
											$output .= '<span class="vcard author post-author">';
												$output .= get_avatar(get_the_author_meta('email'), '24', false, get_the_author_meta('display_name'));
												$output .= '<span class="fn"><a href="'. esc_url(get_author_posts_url(get_the_author_meta('ID'))) .'">'. esc_html(get_the_author_meta('display_name')) .'</a></span>';
											$output .= '</span> ';
										}
				...
				// echo immediately for some plugins
			...
			$output = __('There are no posts on the list.', 'betheme');
		}
