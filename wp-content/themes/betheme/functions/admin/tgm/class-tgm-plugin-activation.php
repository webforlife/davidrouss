 ...
 * @version   2.6.1 for parent theme BeTheme for publication on ThemeForest
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function actions( $install_actions ) {
			...
			if ( $this->is_tgmpa_page() && ! wp_doing_ajax() ) {
				return false;
			}
		...
		protected function is_core_update_page() {
			...
			if ( 'update-core' === $screen->base ) {
				// Core update screen.
				return true;
			} elseif ( 'toplevel_page_betheme' === $screen->base ) {
				// BeTheme dashboard
				return true;
		...
		public function is_tgmpa_complete() {
			...
			// TMP: always show Install Plugins page
			$complete = false;
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					...
					break;
				case 'external':
					$string = __( 'Bundled Premium Plugin', 'tgmpa' );
					break;
				case 'bundled':
		...
		public function column_cb( $item ) {
			...
			return sprintf(
				'<input type="checkbox" name="%1$s[]" value="%2$s" id="%3$s" '. $disabled .' />',
...
*/
		...
		protected function get_row_actions( $item ) {
			...
			if( ! mfn_is_registered() && $item[ 'minimum_version' ] ){
				...
				$action_links[ 'register' ] = '<span style="color:#666">Please</span> <a href="admin.php?page=betheme">register</a> <span style="color:#666">the theme</span>';
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							if ( $this->tgmpa->is_automatic ) {
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = __( '%1$s updated successfully.', 'tgmpa' );
