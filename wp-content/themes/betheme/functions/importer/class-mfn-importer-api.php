...
class Mfn_Importer_API extends Mfn_API {
	...
	public function remote_get_demo(){
		...
		// fallback for WordPress unzip

		$path_content = $path_unzip .'/content.xml.gz';

		if( ! $wp_filesystem->exists( $path_content )
			|| ( $wp_filesystem->exists( $path_content ) && ! $wp_filesystem->get_contents( $path_content ) ) ){

			$wp_filesystem->delete( $path_unzip, true );

			$zip = new ZipArchive;

	    if ( $zip->open( $path_zip ) === TRUE ) {
        $zip->extractTo( $this->path_demo );
        $zip->close();
	    }
		}

    // end: fallback for WordPress unzip
